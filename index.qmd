...
```{r pulling San Francisco map data}
...
```{r pulling race data and wrangling for barplot}
sf_race <- get_acs(
  state = "CA",
  county = "San Francisco",
  geography = "county",
...
colnames(radar_eduaction) <- str_wrap(colnames(radar_education), width = 15)
...
```{r final map, fig.cap= "Plot shows the distribution of the United States White Populaiton by State. Uses ACS 2021 data.", fig.alt= "Map shows the distribution of the United State's White Population. This goes against our suspitions that the trends would continue at a state level."}
